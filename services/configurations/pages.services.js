...
class PageServices {
  ...
  async getListPages({ limit, page, search, ids = [], fields = [], active, hostId, pageId } = {}) {
    const optionsQuery = {
      where: {},
      include: [
    ...
    if (hostId) optionsQuery.where.hostId = hostId;
    if (pageId) optionsQuery.where.pageId = pageId;
  ...
  async getPageDetails({ id, search, fields = [], active, hostId, pageId, includeHistory = false } = {}) {
    const optionsQuery = {
      where: {},
      include: [
        // { model: configHosts, as: 'confighosts' }
