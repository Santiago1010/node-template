...
module.exports = {
  up: async (queryInterface) => {
    ...
    for (let i = 0; i < securityLevels.length; i += batchSize) {
      const batch = securityLevels.slice(i, i + batchSize);
      for (const level of batch) {
        await queryInterface.bulkInsert('config_security_levels', [level], {
          updateOnDuplicate: ['slug', 'name', 'priority', 'description', 'is_default'],
        });
      }
    }
  },
