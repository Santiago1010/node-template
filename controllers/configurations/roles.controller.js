...
// =============================================================================
const RolesServices = require('../../services/configurations/roles.services');
...
class RoleController {
  ...
  static async createRole(req, res, next) {
    try {
      const { name, target, isDefault } = req.body;

      const rolesService = new RolesServices();
      await rolesService.initialize();

      const newrole = await rolesService.createRole(name, { target, isDefault, actor: req.user });
  ...
  static async updateRolesStatus(req, res, next) {
    try {
      ...
      const result = await rolesService.updateRolesStatus(ids, active, { actor: req.user });
  ...
  static async getListRoles(req, res, next) {
    try {
      ...
      const result = await rolesService.getListRoles(req.query);
  ...
  static async getRoleDetails(req, res, next) {
    try {
      ...
      const role = await rolesService.getRoleDetails({ id, ...req.query });
  ...
  static async updateRole(req, res, next) {
    try {
      const { id } = req.params;
      const { name, target, isDefault, active } = req.body;

      const rolesService = new RolesServices();
      await rolesService.initialize();

      const updatedrole = await rolesService.updateRole(id, { name, target, isDefault, active, actor: req.user });
  ...
  static async deleteRole(req, res, next) {
    try {
      ...
      const result = await rolesService.deleteRole(id, { justification, actor: req.user });
